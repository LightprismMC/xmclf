...
/**
 * Map the authorities the runtime reports onto the entries of the account
 * picker. Every authority is offered: the offline account and third-party
 * Yggdrasil services are ordinary options here, not something to be unlocked.
 */
export function buildAuthorityItems(
  ...
  for (const v of authorities) {
    if (v.authority === AUTHORITY_MICROSOFT) {
      ...
      result.push({
        value: v.authority,
        // `serverName` is `''` rather than absent when the service's api root
        // could not be read, so fall back on any falsy value, not just null.
...
export function useAuthorityItems(authorities: Ref<AuthorityMetadata[] | undefined>) {
  const { t } = useI18n()
  return computed(() => buildAuthorityItems(authorities.value ?? [], t)) as Ref<AuthorityItem[]>
}
